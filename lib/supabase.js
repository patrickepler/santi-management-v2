...
export const testConnection = async () => {
  if (!supabase) {
    throw new Error('Supabase not configured. Please set NEXT_PUBLIC_SUPABASE_ANON_KEY in environment variables.');
  }
...
export const db = {
  ...
  async loadAllData() {
    ...
    try {
      ...
      // Load building_sequences
      const { data: seqRaw, error: seqError } = await supabase
        .from('building_sequences')
        .select('*')
        .order('sort_order', { ascending: true });
      
      if (seqError) console.error('Error loading building_sequences:', seqError);

      // Convert sequences to the format page.js expects
      const buildingSequences = { standalone: [] };
      (seqRaw || []).forEach(seq => {
        if (seq.project_id) {
          // Part of a project - group by project
          if (!buildingSequences[seq.project_id]) {
            buildingSequences[seq.project_id] = { zones: [] };
          }
          buildingSequences[seq.project_id].zones.push({
            id: seq.id,
            name: seq.name,
            sortOrder: seq.sort_order
          });
        } else {
          // Standalone
          buildingSequences.standalone.push({
            id: seq.id,
            name: seq.name,
            sortOrder: seq.sort_order
          });
