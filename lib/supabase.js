...
export const db = {
  ...
  async upsertOrderItem(item) {
    ...
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) console.error('Error upserting order item:', error);
    return data;
  },
  ...
  async deleteOrderItem(id) {
    ...
    if (error) console.error('Error deleting order item:', error);
    return !error;
  },

  async getOrderItemsByScTask(scTaskId) {
    if (!supabase) return null;
    const { data, error } = await supabase
      .from('order_items')
      .select('*')
      .eq('sc_task_id', scTaskId)
      .order('sort_order');
    if (error) console.error('Error fetching order items for SC task:', error);
    return data;
  },

  async getOrderItemsByBuildingTask(buildingTaskId) {
    if (!supabase) return null;
    const { data, error } = await supabase
      .from('order_items')
      .select('*')
      .eq('building_task_id', buildingTaskId)
      .order('sort_order');
    if (error) console.error('Error fetching order items for building task:', error);
    return data;
  },
  ...
  async loadAllData() {
    if (!supabase) {
      ...
      return { buildingTasks: [], kanbanTasks: [], recurringTasks: [], workforce: [], options: {}, buildingSequences: null, pendingChanges: [], bugReports: [], comments: {}, profiles: [], orderItems: [] };
    }
    ...
    try {
      ...
      if (oiError) console.error('Error loading order_items:', oiError);

      // Transform order items to page.js format
      const orderItems = (orderItemsRaw || []).map(item => ({
        id: item.id,
        scTaskId: item.sc_task_id,
        buildingTaskId: item.building_task_id,
        name: item.name,
        status: item.status,
      ...
      console.log(`✅ Loaded: ${buildingTasks.length} building tasks, ${kanbanTasks.length} kanban tasks, ${orderItems.length} order items, ${Object.keys(comments).length} comment threads`);
    ...
    try {
      ...
      if (data.orderItems && data.orderItems.length > 0) {
        const rows = data.orderItems.map(item => ({
          id: item.id,
          sc_task_id: item.scTaskId,
          building_task_id: item.buildingTaskId,
          name: item.name,
        ...
        if (error) console.error('Error saving order_items:', error);
      }
...
export const subscribeToAll = (callbacks) => {
  ...
  if (callbacks.orderItems) {
    unsubscribes.push(subscribeToChanges('order_items', callbacks.orderItems));
  }
