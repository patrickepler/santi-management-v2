...
const Dropdown = ({ value, options, onChange, placeholder, allowNew, onAddNew, disabled }) => {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button type="button" onClick={() => !disabled && setOpen(!open)} style={{ width: '100%', padding: '6px 10px', fontSize: '13px', background: disabled ? '#f9fafb' : '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', textAlign: 'left', cursor: disabled ? 'not-allowed' : 'pointer', color: value ? (disabled ? '#9ca3af' : '#1f2937') : '#9ca3af', opacity: disabled ? 0.7 : 1 }}>{value || placeholder}</button>
      {open && !disabled && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', zIndex: 100, maxHeight: '220px', overflowY: 'auto', minWidth: '150px' }}>
          {allowNew && (
            <div style={{ borderBottom: '1px solid #e5e7eb', padding: '8px', background: '#f9fafb' }}>
              <input
                value={newValue}
